...
meta=$(cat ansi2image/__meta__.py | sed "s/__build__.*/__build__ = "${build}"/")
echo "$meta" > ansi2image/__meta__.py

git add ./ansi2image/__meta__.py
...
bin_files = []
if os.path.isdir(f"{here}/ansi2image/fonts"):
    bin_files = [
        os.path.join(dp, f).replace(here.strip('/') + '/', '').lstrip('/. ')
        for dp, dn, filenames in os.walk(f"{here}/ansi2image/fonts") for f in filenames
...
setup(
    ...
    entry_points={'console_scripts': [
        'ansi2image=ansi2image.ansi2image:run',
        ]
    },
    project_urls={
        "Main Author": "https://sec4us.com.br/instrutores/helvio-junior/",
        "Documentation": "https://github.com/helviojunior/ansi2image",
        "Source": "https://github.com/helviojunior/ansi2image",
    },
)